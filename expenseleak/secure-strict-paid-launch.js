...
async function render(){if(!(await ctx()))return;const [{data:g,error},{data:p}]=await Promise.all([sb.from('expenseleak_strict_paid_launch_gate').select('*').eq('business_id',business.id).maybeSingle(),sb.from('expenseleak_commercial_plan_public').select('*').maybeSingle()]);if(error||!g)return;const plan=p||{};const items=[
 ['Manual core E2E','All seven core manual scenarios require evidence.',!!g.manual_core_e2e_ready],
 ['Manual connector + recovery E2E','Connector sync and recovery must both be manually evidenced.',!!g.manual_extended_e2e_ready],
 ['Billing lifecycle manual sign-off','Human verification of checkout, entitlement, failure/recovery and cancellation.',!!g.billing_lifecycle_manual_passed],
 ['CSV pipeline evidence','A completed secure CSV import must have persisted transaction evidence.',!!g.csv_pipeline_ready],
 ['AI document pipeline evidence','A PDF/image analysis must complete and persist contract or finding evidence.',!!g.ai_document_pipeline_ready],
 ['Currency provenance evidence','AI document results must record whether currency came from the document or workspace fallback.',!!g.ai_currency_provenance_ready],
 ['Automated Stripe test evidence','Test-mode webhook history must prove checkout, subscription, failure, recovery and cancellation with zero failed test events.',!!g.automated_billing_e2e_ready],
 ...
 box.innerHTML=`<div class="el-gov-head"><div><h3>Strict Paid Launch Gate</h3><p>This is the final server-backed checklist. No single green CI run, manual checkbox or billing event can unlock live charging on its own.</p></div><div class="el-gov-badges"><span class="el-gov-badge">Prerequisites: ${strict?'passed':'blocked'}</span><span class="el-gov-badge">Live checkout: ${live?'unlocked':'locked'}</span><span class="el-gov-badge">${clean(plan.product_name||'ExpenseLeak AI Pro')}</span></div></div><div class="el-gov-list">${items.map(x=>row(...x)).join('')}</div><div class="el-gov-note" style="margin-top:12px"><b>Fail-closed policy:</b> the database re-checks strict prerequisites when live billing records are written. If evidence later becomes stale or invalid, live billing writes are blocked again. Current live checkout state: <b>${live?'UNLOCKED':'LOCKED'}</b>.</div>`;
}
