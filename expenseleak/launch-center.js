...
function injectPublic(){
 ...
 const p=document.querySelector('#pricing .section-title p');if(p)p.textContent='Public beta is free to test. Paid checkout stays disabled until billing, entitlement and legal launch checks pass.';
...
async function counts(){const bid=business.id;const [docs,tx,find,contracts,conn]=await Promise.all([
 ...
 ]);return{docs:docs.count||0,tx:tx.count||0,find:find.count||0,contracts:contracts.count||0,conn:conn.count||0}}
async function render(){if(!(await ctx()))return;const c=await counts();const steps=[
 ...
 const done=steps.filter(x=>x[1]).length;let box=document.querySelector('#elLaunchCenter');if(!box){box=document.createElement('div');box.id='elLaunchCenter';box.className='preview-box el-gov';const anchor=document.querySelector('#elConnectorCenter')||document.querySelector('#elAdvancedIntel')||document.querySelector('#elWorkspace')||document.querySelector('#audit');if(anchor&&anchor!==document.body)anchor.insertAdjacentElement('afterend',box);else(document.querySelector('.wrap')||document.body).appendChild(box)}
 box.innerHTML=`<div class="el-gov-head"><div><h3>Launch & Onboarding Center</h3><p>A simple path from first sign-in to a useful, verified spend review.</p></div><div class="el-gov-badges"><span class="el-gov-badge">${done}/${steps.length} onboarding steps</span><span class="el-gov-badge">Public beta</span></div></div><div class="el-gov-list">${steps.map(([t,ok,n],i)=>`<div class="el-gov-row"><div class="el-gov-row-head"><div><b>${i+1}. ${clean(t)}</b><small>${clean(n)}</small></div><span class="el-gov-status ${ok?'ok':'pending'}">${ok?'done':'next'}</span></div></div>`).join('')}</div><div class="el-gov-note" style="margin-top:12px">Need help? <a href="support.html" style="color:#71d7ff">Open Support & Data Requests</a>. Privacy, Terms and Security pages are available publicly before paid launch.</div>`;
}
