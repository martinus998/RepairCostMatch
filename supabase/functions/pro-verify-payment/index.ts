...
const EXPECTED_PAYMENT_LINK = "plink_1UFIYlBVUFmkZjNknKTEG0FR";
...
Deno.serve(async (req: Request) => {
  ...
  if (!/^cs_test_[A-Za-z0-9_]+$/.test(sessionId)) return json({ error: "invalid_session" }, 400, origin);

  const stripeSecret = Deno.env.get("STRIPE_SECRET_KEY");
  if (!stripeSecret || !stripeSecret.startsWith("sk_test_")) return json({ error: "billing_not_configured" }, 503, origin);

  const stripeRes = await fetch(`https://api.stripe.com/v1/checkout/sessions/${encodeURIComponent(sessionId)}`, { headers: { "Authorization": `Bearer ${stripeSecret}` } });
  if (!stripeRes.ok) return json({ error: "stripe_verification_failed" }, 400, origin);
  const session = await stripeRes.json();

  const valid = session?.livemode === false && session?.status === "complete" && session?.payment_status === "paid" && session?.mode === "payment" && session?.payment_link === EXPECTED_PAYMENT_LINK && session?.amount_total === EXPECTED_AMOUNT && session?.currency === EXPECTED_CURRENCY && session?.metadata?.project === "RepairCostMatch" && session?.metadata?.tier === "pro";
  ...
  const { error } = await supabase.from("pro_entitlements").upsert({
  ...
  return json({ ok: true, entitlement_token: token, tier: "pro", environment: "test" }, 200, origin);
});
